...
const Dashboard = () => {
    ...
    const memoizedNotificationCount = useMemo(() => notificationCount, [notificationCount]);
    const memoizedTemperature = useMemo(() => temperature, [temperature]);
    const memoizedHumidity = useMemo(() => humidity, [humidity]);
    const memoizedDistance = useMemo(() => distance, [distance]);
    ...
    // handle openGate
    const handle_Open_Gate = async (e) => {
        try {
            const { data } = await axios.get('http://localhost:8000/api/gateroute/opengate');
            if (data.status === false)
                throw new Error(data.message);
            setIsGateOpen(true);
        } catch (error) {
    ...
    // hanlde close
    const handle_Close_Gate = async (e) => {
        try {
            try {
    ...
    // socket logic
    useEffect(() => {
        console.log({ 'socketlogic': 'socketlogic' });
        if (!user) {
            return navigate('/');
        }
        // temperature data
        ...
        <DashApp>
            <Box margin={'20px'}>
                ...
                >
                    ...
                    >
                        <StatBox
                            title={memoizedTemperature + ' °C'}
                            subtitle="Temperature"
                            progress={progressConverter(memoizedTemperature, 0, 100)}
                            // increase="+21%"
                            icon={
                    ...
                    >
                        <Box>
                            <Box
                                ...
                                padding={1}
                            >
                                {isGateOpen ? <LockOpenIcon
                                    sx={{ color: colors.blueAccent[400], fontSize: "26px" }}
                                /> : <LockIcon
                                    sx={{ color: colors.blueAccent[400], fontSize: "26px" }}
                                />}
                                <Typography variant='h5' ml={1}> Gate Controller</Typography>
                                <Typography variant='h5' ml={4} color={'yellow'}> Gate-Status</Typography>
                            </Box>
                            {/* gate logic */}
                            <Box
                                display={'flex'}
                            ...
                            >
                                <Box>
                                    {!isGateOpen ? <Button
                                        ...
                                        onClick={handle_Open_Gate}


                                    >
                                        <LockOpenIcon sx={{ mr: "10px" }} />
                                        Open Gate
                                    </Button> : <Button
                                        sx={{
                                            backgroundColor: colors.blueAccent[700],
                                ...
                                >
                                    {isGateOpen ? <img
                                        height={'70px'}
                                        width={'70px'}
                                        src={require('./Doorpng/open.png')}
                                    /> : <img
                                        height={'70px'}
                                        width={'70px'}
                                        src={require('./Doorpng/close.png')}
                                    />}
                    ...
                    >
                        ...
                        >
                            ...
                            <Typography
                                variant="h5"
                                color={memoizedDistance < 20 ? colors.redAccent[500] : colors.blueAccent[400]}
                                sx={{ mt: "15px" }}
                            >
                                {memoizedDistance + ' Cm'}
                            </Typography>

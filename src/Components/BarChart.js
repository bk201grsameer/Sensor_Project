...
const BarChart = ({ isDashboard = false }) => {
    ...
    const [barDataArr, setBarDataArr] = useState([]);
    const fetchBarData = async () => {
        try {
            setLoading(true);
            const { data } = await axios.get('http://localhost:8000/api/chart/barchart');
            if (data.status == false)
                throw new Error(data.message);
            setLoading(false);
            console.log(data.message);
            setBarDataArr(data.message);
        } catch (error) {
            console.log('error');
            setLoading(false);
        }
    ...
    return (
        loading ? <Loading /> :
            <ResponsiveBar
                ...
                // this will decide what data to render
                keys={[
                    'distance',
                    'humidity',
                    'temperature',
